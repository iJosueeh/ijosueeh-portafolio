...
export const SlotTabs: React.FC<SlotTabsProps> = ({
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-1 sm:gap-2 mb-2 sm:mb-3 border-b border-purple-900/60 pb-1.5 sm:pb-2.5">
      {slots.map((slot, sIdx) => {
        ...
        return (
          <button
            ...
            type="button"
            className={`p-1.5 sm:p-2 flex flex-col text-left transition-all cursor-pointer relative ${
              isActive
                ? 'bg-gradient-to-b from-[#251545] to-[#150e2a] border-2 border-pink-300 shadow-[0_0_10px_rgba(244,63,133,0.3)]'
          ...
          >
            ...
            <div className="flex items-center justify-between w-full mb-0.5">
              <span className={`text-[7px] sm:text-[8px] font-arcade ${isActive ? 'text-pink-300' : 'text-purple-300/60'}`}>
                [{sIdx + 1}] {slot.slotNumber}
              </span>
              <span className="text-[10px] sm:text-xs">{slot.icon}</span>
            </div>
            <div
              className={`font-arcade text-[8px] sm:text-[9px] md:text-[10px] tracking-wider truncate ${
